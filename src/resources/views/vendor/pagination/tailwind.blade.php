@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div>
                <span class="pagination__inner relative z-0 inline-flex shadow-sm rounded-md">
                    {{-- Previous Page Link --}}
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span class="pagination__item-current" aria-current="page">
                                        <span class="item-current__span relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-default leading-5">{{ $page }}</span>
                                    </span>
                                @else
